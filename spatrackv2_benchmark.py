...
def spatrackv2_inference(video_tensor, query_pts):
    """
    ...
    with torch.no_grad():
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ...
            predictions = vggt4track_model(video_tensor.cuda())
    ...
    with torch.amp.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        data_npz_load["extrinsics"] = (c2w_traj).cpu().numpy()
...
def get_args_parser():
    ...
    parser.add_argument("--img_height", default=294, type=int, help="Image height")
...
def main():
    ...
    # Save metrics to txt
    metrics_file = os.path.join(f"{args.dataset}_spatrackv2_benchmarking_metrics.txt")
